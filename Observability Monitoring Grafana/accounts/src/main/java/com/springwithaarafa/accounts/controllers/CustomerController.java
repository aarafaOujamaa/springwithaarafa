...
public class CustomerController {
    ...
    // correlationId sent by my gateWayServer
    public ResponseEntity<CustomerDetailsDto> fetchCustomerDetails(@RequestHeader("ms-correlation-id") String correlationId,
                                                                   @RequestParam
                                                                   @Pattern(regexp="(^$|[0-9]{10})",message = "Mobile number must be 10 digits")
                                                                   String mobileNumber){
        logger.debug("fetchCustomerDetails methhod is started");
        CustomerDetailsDto customerDetailsDto = iCustomersService.fetchCustomerDetails(correlationId,mobileNumber);
        logger.debug("fetchCustomerDetails methhod is ended");
